...
use harness_model::{ZhipuProvider, *};
...
#[tokio::test]
async fn zhipu_stream_captures_reasoning_content_as_private_continuation() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                ...
                .set_body_raw(
                    concat!(
                        "data: {\"id\":\"glm_1\",\"choices\":[{\"index\":0,\"delta\":{\"reasoning_content\":\"think \"},\"finish_reason\":null}]}\n\n",
                        "data: {\"id\":\"glm_1\",\"choices\":[{\"index\":0,\"delta\":{\"reasoning_content\":\"again\",\"content\":\"answer\"},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":3,\"completion_tokens\":2}}\n\n",
                        "data: [DONE]\n\n",
                    ),
                    "text/event-stream",
                ),
    ...
    assert!(events.iter().any(|event| matches!(
        ...
        ModelStreamEvent::ProviderContinuationDelta {
            ...
            payload,
        } if payload == &zhipu_payload("think again")
    )));
}

#[tokio::test]
async fn zhipu_non_stream_captures_reasoning_content_as_private_continuation() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
    ...
    assert!(events.iter().any(|event| matches!(
        ...
        ModelStreamEvent::ProviderContinuationDelta {
            kind: ProviderContinuationKind::ReasoningReplay,
            payload,
        } if payload == &zhipu_payload("private chain")
    )));
}

#[tokio::test]
async fn zhipu_second_request_replays_reasoning_content_for_assistant_tool_call() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    req.provider_context.continuations = vec![record(assistant_id, zhipu_payload("saved trace"))];
...
#[tokio::test]
async fn zhipu_missing_required_reasoning_continuation_fails_closed_before_request() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
...
#[tokio::test]
async fn zhipu_invalid_reasoning_continuation_fails_closed_before_request() {
    let assistant_id = MessageId::new();
    let invalid_cases = vec![
        record(MessageId::new(), zhipu_payload("saved trace")),
        record_with_kind(
            assistant_id,
            ProviderContinuationKind::ToolReplay,
            zhipu_payload("saved trace"),
        ),
        record(
            assistant_id,
            json!({
        ...
        record(
            assistant_id,
            json!({
                "format": "zhipu.reasoning-content.v1",
                "reasoningContent": "saved trace",
            }),
        ),
        record(
            assistant_id,
            json!({
                "format": "zhipu.reasoning_content.v1",
            }),
        ),
        record(assistant_id, zhipu_payload("")),
    ];
...
fn provider(server: &MockServer) -> ZhipuProvider {
    ZhipuProvider::from_api_key("provider-key").with_base_url(server.uri())
}
...
fn record_with_kind(
    ...
    ProviderContinuationRecord {
        ...
        protocol: ModelProtocol::ChatCompletions,
        dialect: "openai_chat.zhipu".to_owned(),
...
fn zhipu_payload(reasoning: &str) -> Value {
    json!({
        "format": "zhipu.reasoning_content.v1",
