...
use harness_model::{
    ...
    ModelRuntimeSemantics, ProviderBuildConfig, ProviderRegistryError, ProviderServiceExecution,
...
fn minimax_model_catalog_matches_official_protocol_parameters_and_pricing() {
    ...
    for parameter in [
        ...
        assert!(
            m3.supported_parameters
                .iter()
                .any(|value| value == parameter),
            "MiniMax-M3 should support {parameter}"
        );
    }
    let pricing = m3
        .pricing
    ...
    for parameter in [
        ...
        assert!(
            m27.supported_parameters
                .iter()
                .any(|value| value == parameter),
            "MiniMax-M2.7 should support {parameter}"
        );
    }
    let pricing = m27
        .pricing
