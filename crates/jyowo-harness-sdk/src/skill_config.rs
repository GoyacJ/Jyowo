...
pub fn apply_skill_config_statuses(
    ...
    for skill in registry_snapshot.entries.values() {
        let missing = missing_required_config_for_status(skill, config_snapshot);
        if missing.is_empty() {
            continue;
        }
...
fn missing_required_config_for_status(
    ...
    for declaration in &skill.frontmatter.config {
        ...
        // Status assembly is a per-skill isolation boundary. Keep the typed
        // error on direct secret access, but expose only an unavailable
        // prerequisite here so one keychain failure cannot block the registry.
        let configured = is_configured(skill, declaration, config_snapshot).unwrap_or(false);
        if !configured {
