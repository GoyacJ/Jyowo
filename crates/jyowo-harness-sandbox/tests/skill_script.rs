...
#[cfg(all(feature = "local", any(target_os = "linux", target_os = "macos")))]
...
#[tokio::test]
async fn bubblewrap_executes_in_a_private_child_of_the_local_sandbox_root() {
    let bwrap_available = std::env::var_os("PATH").is_some_and(|path| {
        std::env::split_paths(&path).any(|directory| directory.join("bwrap").is_file())
    });
    if !bwrap_available {
        return;
    }
    ...
    let sentinel = workspace.path().join("host-workspace-secret");
    std::fs::write(&sentinel, "must not be visible").unwrap();
    let mut request = request(script_decl());
    request.files[0].content = format!(
        "if test -e '{}'; then printf exposed; exit 7; fi\nprintf ok\n",
        sentinel.display()
    );
    let backend =
        Arc::new(LocalSandbox::new(workspace.path()).with_isolation(LocalIsolation::Bubblewrap));
    let mut context = test_context();
    context.workspace_root = workspace.path().to_path_buf();

    let result = execute_skill_script(backend, request, context)
        .await
