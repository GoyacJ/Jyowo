...
impl TempSkillWorkspace {
    fn create(workspace_root: &Path) -> Result<Self, SandboxError> {
        ...
        let directory = if workspace_root.as_os_str().is_empty() {
            builder.tempdir()
        } else {
            builder.tempdir_in(workspace_root)
        }
        .map_err(io_error)?;
        Ok(Self { directory })
    }
...
mod temp_skill_workspace_tests {
    ...
    #[test]
    fn workspace_is_atomic_private_and_scoped_to_the_caller_workspace() {
        ...
        assert!(path.starts_with(caller_workspace.path()));
