...
impl SkillRegistry {
    ...
    pub fn try_replace_sources<E, F>(
        &self,
        replacements: Vec<(SkillSource, Vec<Skill>)>,
        reconcile: F,
    ) -> Result<SkillRegistrySnapshot, SkillRegistryUpdateError<E>>
    where
        F: FnOnce(&SkillRegistrySnapshot, &SkillRegistrySnapshot) -> Result<(), E>,
        ...
        self.try_update(
            move |next| {
                ...
                for candidates in next.candidates.values_mut() {
                    candidates.retain(|skill| !replaced.contains(&skill.source));
                }
                next.candidates
                    .retain(|_, candidates| !candidates.is_empty());
                for (source, skills) in replacements {
