...
| `jyowo-harness-mcp` | `crates/jyowo-harness-mcp` | L3 | Owns MCP connection state, tool injection, resource updates, sampling, elicitation, and MCP authorization bridging. |
...
| `jyowo-harness-session` | `crates/jyowo-harness-session` | L3 | Owns sessions, workspace bootstrap, stream handles, session lifecycle, and session turn runtime assembly. |
...
wins over the saved workspace default for that run only. If the request omits
it, Rust reads the saved default. `Auto` must still be validated by the Rust
shell and fail closed when the desktop build does not support auto mode.
...
the `run.started` projection payload include the run model snapshot and
resolved permission mode so Replay and Activity show the run snapshot. The
timeline projection exposes only UI-safe model fields:
`modelConfigId`, `providerId`, `modelId`, `displayName`, and `protocol`.
...
[ ] credential resolver passes run model config id or operation id plus route kind
