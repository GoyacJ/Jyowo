...
use harness_contracts::{
    ModelProtocol, ModelRequestOptions, NoopRedactor, SkillConfigDocument, SkillStatus,
};
use harness_skill::{
...
#[tokio::test]
async fn global_skill_commit_reloads_replacement_and_shared_previous_runtimes() {
    let root = tempfile::tempdir().unwrap();
    ...
    .unwrap();
    let mut state_three = DesktopRuntimeState::with_settings_runtime_for_workspace(
        workspace_two.clone(),
        runtime_two.clone(),
    )
    .unwrap();
    ...
    state_two.set_skill_store_for_test(skill_store.clone());
    state_three.set_skill_store_for_test(skill_store);
    state_one.set_config_stores_for_test(GlobalConfigStore::new(layout.clone()), None);
    state_two.set_config_stores_for_test(GlobalConfigStore::new(layout.clone()), None);
    state_three.set_config_stores_for_test(GlobalConfigStore::new(layout), None);

    let replacement_runtime_two = test_settings_runtime(&workspace_two).await;
    state_two.replace_settings_runtime(
        replacement_runtime_two.clone(),
        "test-model".to_owned(),
    ...
    assert!(runtime_two
        ...
        .unwrap()
        .is_none());
    assert!(replacement_runtime_two
        .view_runtime_skill("shared-runtime-test", false)
        .unwrap()
        .is_none());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn skill_store_mutations_wait_for_an_in_flight_runtime_rebuild() {
    let root = tempfile::tempdir().unwrap();
    let workspace = root.path().canonicalize().unwrap();
    let layout = test_layout(&workspace);
    let snapshot_load_gate = Arc::new(BlockingLoadGate::default());
    let hook_gate = snapshot_load_gate.clone();
    let store = Arc::new(
        DesktopSkillConfigStore::new(layout.clone(), Arc::new(MemorySecretStore::default()))
            .with_snapshot_load_hook_for_test(Arc::new(move || hook_gate.block_if_armed())),
    );
    let mut state = runtime_state_with_skill_config_store_for_test(workspace.clone(), store)
        .await
        .unwrap();
    state.set_skill_store_for_test(Arc::new(DesktopSkillStore::global(layout.clone())));
    state.set_config_stores_for_test(GlobalConfigStore::new(layout), None);

    let source = workspace.join("serialized-skill-source");
    std::fs::create_dir_all(&source).unwrap();
    std::fs::write(
        source.join("SKILL.md"),
        "---\nname: serialized-skill\ndescription: Serialized skill\n---\nBody.\n",
    )
    .unwrap();
    ...
    let rebuild = tokio::spawn(async move {
        reload_desktop_settings_runtime_after_plugin_change_for_test(&rebuild_state).await
    });
    tokio::task::spawn_blocking(move || {
        snapshot_load_entered
            .recv_timeout(std::time::Duration::from_secs(5))
    ...
    let mut import = tokio::spawn(async move {
        import_skill_with_runtime_state(ImportSkillRequest { source_path }, &import_state).await
    });
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(100), &mut import)
            .await
            .is_err(),
        "skill import bypassed the runtime rebuild lock"
    );
    release_snapshot_load.send(()).unwrap();
    rebuild.await.unwrap().unwrap();
    let imported = import.await.unwrap().unwrap();
    let skill_id = imported.skill.id;
    assert!(state
        .settings_runtime()
        .unwrap()
        .view_runtime_skill("serialized-skill", false)
        .unwrap()
        .is_some());
    ...
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(100), &mut disable)
            .await
            .is_err(),
        "skill toggle bypassed the runtime rebuild lock"
    );
    release_snapshot_load.send(()).unwrap();
    rebuild.await.unwrap().unwrap();
    assert!(!disable.await.unwrap().unwrap().skill.enabled);
    ...
    let mut delete = tokio::spawn(async move {
        delete_skill_with_runtime_state(DeleteSkillRequest { id: skill_id }, &delete_state).await
    });
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(100), &mut delete)
            .await
            .is_err(),
        "skill delete bypassed the runtime rebuild lock"
    );
    release_snapshot_load.send(()).unwrap();
    rebuild.await.unwrap().unwrap();
    assert_eq!(delete.await.unwrap().unwrap().status, "deleted");
}
...
#[tokio::test]
async fn skill_config_mutation_refreshes_all_live_project_runtime_snapshots() {
    let root = tempfile::tempdir().unwrap();
    let workspace_one = root.path().join("one");
    let workspace_two = root.path().join("two");
    std::fs::create_dir_all(&workspace_one).unwrap();
    ...
    for runtime in [&runtime_one, &runtime_two] {
        let skill = parse_skill_markdown(
            "---\nname: shared-configured\ndescription: Shared configured skill\nconfig:\n  - key: region\n    type: string\n    required: true\n---\nUse ${config.region}.\n",
            SkillSource::Workspace("data/skills".into()),
            None,
            SkillPlatform::Macos,
        )
    ...
    .unwrap();
    let skill_store = Arc::new(DesktopSkillStore::global(layout.clone()));
    state_one.set_skill_store_for_test(skill_store.clone());
    state_two.set_skill_store_for_test(skill_store);
    state_one.set_config_stores_for_test(GlobalConfigStore::new(layout.clone()), None);
    state_two.set_config_stores_for_test(GlobalConfigStore::new(layout.clone()), None);
    state_one.set_skill_config_store_for_test(Arc::new(DesktopSkillConfigStore::new(
        layout.clone(),
        secret_store.clone(),
    )));
    state_two.set_skill_config_store_for_test(Arc::new(DesktopSkillConfigStore::new(
        layout,
        secret_store,
    )));
    ...
    for runtime in [&runtime_one, &runtime_two] {
        assert!(matches!(
            runtime
                .view_runtime_skill("shared-configured", false)
                .unwrap()
                .unwrap()
                .summary
                .status,
    ...
    set_skill_config_value_with_runtime_state(
        SetSkillConfigValueRequest {
            skill_id: "workspace:shared-configured".to_owned(),
            key: "region".to_owned(),
            value: json!("cn-east"),
        },
        &state_one,
